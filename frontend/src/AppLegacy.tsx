...
// Standing Nearby uses this exact radius too, so "live reach" and "total footfall" mean the same area.
...
function BillboardMesh({b,onSelect,nearCount,totalVisitors,bidder}:{b:Billboard;onSelect:(b:Billboard)=>void;nearCount:number;totalVisitors:number;bidder?:BidderInfo}) {
 ...
 if(isWall || isVertical) {
   return <RigidBody type="fixed" colliders={false} position={b.position} rotation={[0,b.rotationY ?? 0,0]}>
     ...
     <group onClick={(e)=>{e.stopPropagation();onSelect(b)}}>
       ...
       {bidder?.imageUrl
  ? <AdCreative url={bidder.imageUrl} w={w} h={h}/>
  : !bidder
    ? <AdCreative url={BILLBOARD_PLACEHOLDER_VERTICAL} w={w} h={h}/>
    : null}
 ...
 return <RigidBody type="fixed" colliders={false} position={b.position} rotation={[0,b.rotationY ?? 0,0]}>
   ...
   <group onClick={(e)=>{e.stopPropagation();onSelect(b)}}>
     ...
     {bidder?.imageUrl
  ? <AdCreative url={bidder.imageUrl} w={w-.35} h={h-.35} z={.231}/>
  : !bidder
    ? <AdCreative url={BILLBOARD_PLACEHOLDER_HORIZONTAL} w={w-.35} h={h-.35} z={.231}/>
    : null}
...
function App(){
 ...
 useEffect(()=>{fetch(api+'/api/bookings/active').then(readApi).then((rows:any)=>{if(rows&&typeof rows==='object')setActiveBookings(rows);}).catch(()=>{})},[api]);
 useEffect(()=>{
   if(!authOpen)return;
