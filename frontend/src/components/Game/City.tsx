...
import { Bench, Building, StreetLight, ThemedSky, Tree, WorldFence } from './index';

export type TimeMode = 'morning' | 'evening' | 'night';

export const TIME_THEMES: Record<TimeMode, {
  bg: string;
  fog: string;
  ground: string;
  road: string;
  ...
  evening: { bg: '#9b6b72', fog: '#a77a79', ground: '#52605c', road: '#252934', ambient: 0.72, sun: 1.45, sunColor: '#ffb27a', sunPos: [35, 22, -25] },
  night: { bg: '#07111e', fog: '#07111e', ground: '#172233', road: '#090f1a', ambient: 0.5, sun: 1.45, sunColor: '#dcecff', sunPos: [25, 55, 18] },
};
...
export function City({ timeMode, billboards = [] }: { timeMode: TimeMode; billboards?: Billboard[] }) {
  ...
  return <group>
    ...
    <directionalLight castShadow position={theme.sunPos} intensity={theme.sun} color={theme.sunColor} shadow-mapSize={[1024,1024]} />
    ...
    <Bench position={[-13,0,26]} />
    <Bench position={[13,0,-8]} />
  </group>;
}
