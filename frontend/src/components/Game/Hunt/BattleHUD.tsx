...
export default function BattleHUD({ phase, stats, players, timeLeft, status, hitMarker, killFeed, onStart, onExit, onReload }: Props) {
  const ordered = [...players].sort((a, b) => b.kills - a.kills || a.deaths - b.deaths);
  const minutes = Math.floor(timeLeft / 60).toString().padStart(2, "0");
  const seconds = Math.floor(timeLeft % 60).toString().padStart(2, "0");
  const opponents = players.filter((p) => p.alive).length - 1;
  ...
  return (
    <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 999, color: "#fff", fontFamily: "Inter,system-ui,sans-serif" }}>
      <div style={{ position: "absolute", top: 16, left: "50%", transform: "translateX(-50%)", display: "flex", gap: 10, alignItems: "center" }}>
        ...
        <div style={{ padding: "9px 13px", borderRadius: 12, background: "rgba(20,25,35,.88)", border: "1px solid rgba(255,255,255,.12)", fontSize: 12 }}>{opponents > 0 ? `${status} · ${opponents} opponent${opponents === 1 ? "" : "s"}` : "WAITING FOR OPPONENT"}</div>
      </div>
      ...
      <div style={{ position: "absolute", top: 16, right: 16, width: 205, padding: 12, borderRadius: 14, background: "rgba(5,10,18,.86)", border: "1px solid rgba(255,255,255,.12)" }}>
        ...
        {ordered.slice(0, 6).map((p, i) => <div key={p.id} style={{ display: "flex", justifyContent: "space-between", gap: 8, padding: "4px 0", opacity: p.alive ? 1 : .5, fontSize: 12 }}><span>{i + 1}. {p.name}</span><b>{p.kills}</b></div>)}
      </div>

      <div style={{ position: "absolute", top: 82, left: 18, display: "flex", gap: 8 }}><Stat label="KILLS" value={stats.kills} /><Stat label="DEATHS" value={stats.deaths} /><Stat label="STREAK" value={stats.streak} /></div>
      ...
      <div style={{ position: "absolute", right: 18, bottom: 16, textAlign: "right" }}><div style={{ fontSize: 36, fontWeight: 1000, lineHeight: 1 }}>{stats.ammo}<span style={{ fontSize: 14, opacity: .55 }}> / 30</span></div><div style={{ fontSize: 10, opacity: .65 }}>CLICK TO SHOOT · R TO RELOAD</div></div>

      {phase === "playing" && <div style={{ position: "absolute", left: "50%", top: "50%", transform: "translate(-50%,-50%)", width: hitMarker ? 34 : 24, height: hitMarker ? 34 : 24, transition: "all 80ms ease" }}>
        <span style={{ position: "absolute", left: hitMarker ? 16 : 11, top: 0, width: 2, height: hitMarker ? 34 : 24, background: hitMarker ? "#ffdf5a" : "rgba(255,255,255,.95)" }} />
        <span style={{ position: "absolute", left: 0, top: hitMarker ? 16 : 11, width: hitMarker ? 34 : 24, height: 2, background: hitMarker ? "#ffdf5a" : "rgba(255,255,255,.95)" }} />
      </div>}
      ...
      {(phase === "ready" || phase === "finished") && <div style={{ position: "absolute", inset: 0, pointerEvents: "auto", background: "rgba(3,7,13,.68)", display: "grid", placeItems: "center" }}>
        <div style={{ width: "min(520px,90vw)", padding: 28, borderRadius: 20, background: "linear-gradient(145deg,rgba(16,25,40,.98),rgba(5,9,16,.98))", border: "1px solid rgba(255,255,255,.14)", boxShadow: "0 25px 80px rgba(0,0,0,.5)", textAlign: "center" }}>
          <div style={{ fontSize: 12, letterSpacing: 3, opacity: .55 }}>URBANCITY</div><h1 style={{ margin: "7px 0", fontSize: 38 }}>BATTLE MODE</h1>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "center", marginTop: 18 }}><button onClick={onExit} style={buttonStyle}>EXIT</button><button onClick={onStart} style={{ ...buttonStyle, background: "linear-gradient(135deg,#4b6bff,#705cff)", color: "#fff", border: 0 }}>{phase === "finished" ? "PLAY AGAIN" : "START BATTLE"}</button></div>
        </div>
      </div>}
...
function Stat({ label, value }: { label: string; value: number }) { return <div style={{ padding: "7px 11px", borderRadius: 10, background: "rgba(5,10,18,.8)", border: "1px solid rgba(255,255,255,.1)" }}><div style={{ fontSize: 9, opacity: .55 }}>{label}</div><b style={{ fontSize: 17 }}>{value}</b></div>; }
