...
import type { Remote } from '../../types/player';
    ...
    <div className="minimap">
      <div className="mapgrid">
        ...
        {[-44, -28, -12, 12, 28, 44].map((v, i) => (
          <div
            key={`rv${i}`}
            className="minor-road v"
            style={{ left: `${((v + 60) / 120) * 100}%` }}
          />
        ))}
        ...
        {[-44, -28, -12, 12, 28, 44].map((v, i) => (
          <div
            key={`rh${i}`}
            className="minor-road h"
            style={{ top: `${((v + 60) / 120) * 100}%` }}
          />
        ))}
        ...
        {MAP_BILLBOARDS.map((b) => (
          <i
            key={b.id}
            className="map-billboard"
            title={`Billboard #${b.id}`}
            style={toPct(b.position[0], b.position[2])}
          />
        ))}
        ...
        {players.map((p) => (
          <i
            ...
            style={toPct(p.position[0], p.position[2])}
          />
        ))}
