...
const MAP_BILLBOARDS: Billboard[] = [
  ...
  { id:'W01', type:'Building Wall', kind:'wall-ad', position:[-20,7.8,-34.47], traffic:'High', bid:3200, occupied:false, ad:'ADVERTISE HERE', size:[5.8,3.4] },
  { id:'W02', type:'Building Wall', kind:'wall-ad', position:[20,8.8,-24.47], traffic:'High', bid:4500, occupied:false, ad:'CITY REACH', size:[7.2,4.2] },
...
function GameCamera(){
 const yaw=useRef(0), pitch=useRef(0.32), distance=useRef(11);
...
function BillboardMesh({b,onSelect,nearCount,totalVisitors,bidder}:{b:Billboard;onSelect:(b:Billboard)=>void;nearCount:number;totalVisitors:number;bidder?:BidderInfo}) {
 ...
 if(isWall || isVertical) {
   return <RigidBody type="fixed" colliders={false} position={b.position} rotation={[0,b.rotationY ?? 0,0]}>
     ...
     <Text position={[0,h/2+.62,.15]} fontSize={.22} color="#8ff0b3" anchorX="center" anchorY="middle" outlineWidth={0.018} outlineColor="#07111e">footfall</Text>
 ...
 return <RigidBody type="fixed" colliders={false} position={b.position} rotation={[0,b.rotationY ?? 0,0]}>
   ...
   <group onClick={(e)=>{e.stopPropagation();onSelect(b)}}>
     ...
     <Text position={[0,h/2+.62,.5]} fontSize={.22} color="#8ff0b3" anchorX="center" anchorY="middle" outlineWidth={0.018} outlineColor="#07111e">footfall</Text>
   </group>
...
function App(){
 ...
 const [adFile,setAdFile]=useState<File|null>(null),[adTitle,setAdTitle]=useState(''),[adUrl,setAdUrl]=useState(''),[bookingCompanyName,setBookingCompanyName]=useState(''),[uploadBusy,setUploadBusy]=useState(false); const [editMode,setEditMode]=useState(false),[editBusy,setEditBusy]=useState(false),[removePhoto,setRemovePhoto]=useState(false);
 ...
 const loadMe=async()=>{const r=await fetch(api+'/api/auth/me',{headers:authHeaders()});if(!r.ok)throw new Error('Session expired');const me=await r.json();setUser(me);if(me.wallet?.balance!=null)setBalance(Number(me.wallet.balance));};
 useEffect(()=>{if(localStorage.getItem('urbancity_token'))loadMe().catch(()=>localStorage.removeItem('urbancity_token'));},[]);
 useEffect(()=>{const t=window.setInterval(()=>setClock(Date.now()),1000);return()=>window.clearInterval(t)},[]);
 useEffect(()=>{fetch(api+'/api/bookings/history').then(readApi).then((rows:any[])=>{if(!Array.isArray(rows))return;const now=Date.now();const active:Record<string,any>={};for(const x of rows){if(x.status==='ACTIVE'&&new Date(x.endDate).getTime()>now&&(!active[x.billboardId]||new Date(active[x.billboardId].endDate)<new Date(x.endDate)))active[x.billboardId]=x}setActiveBookings(active);}).catch(()=>{})},[]);
 useEffect(()=>{
   if(!authOpen)return;
 ...
 const readApi=async(r:Response)=>{const text=await r.text();try{return JSON.parse(text)}catch{return {error:text||('Request failed ('+r.status+')')}}};
 const submitAuth=async()=>{setAuthError('');setAuthBusy(true);try{const body=authMode==='login'?{email:authEmail,password:authPassword}:{email:authEmail,password:authPassword,username:authUsername,displayName:authUsername,websiteUrl:authWebsite||undefined};const r=await fetch(api+'/api/auth/'+(authMode==='login'?'login':'register'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const data=await readApi(r);if(!r.ok)throw new Error(data.error||'Authentication failed');localStorage.setItem('urbancity_token',data.token);await loadMe();setAuthOpen(false);setAuthPassword('');setAuthWebsite('');}catch(e:any){setAuthError(e.message||'Authentication failed');}finally{setAuthBusy(false);}};
 const logout=()=>{localStorage.removeItem('urbancity_token');setUser(null);setBalance(1000);};
 const isMain=(b:Billboard)=>b.type==='Premium Road';
 const bookingPrice=(b:Billboard,m:number)=>{if(!isMain(b))return (m/30)*1.05;if(m===1440)return 8.40;if(m>1440)return 8.40+((m-1440)/30)*0.19;return (m/30)*0.21;};
 const remaining=(end?:string)=>{if(!end)return '';let s=Math.max(0,Math.ceil((new Date(end).getTime()-clock)/1000));const d=Math.floor(s/86400);s%=86400;const h=Math.floor(s/3600);s%=3600;const m=Math.floor(s/60);s%=60;return (d?d+'d ':'')+String(h).padStart(2,'0')+'h '+String(m).padStart(2,'0')+'m '+String(s).padStart(2,'0')+'s'};
 ...
 const loadHistory=async()=>{try{const r=await fetch(api+'/api/bookings/history');const d=await readApi(r);if(r.ok)setBookingHistory(d)}catch{}};
 ...
 const book=async()=>{if(!selected)return;if(!user){setAuthOpen(true);setAuthError('Login or register to book advertising space.');return;}setBookingBusy(true);setBookingError('');try{const advertisementId=await uploadCreative();const r=await fetch(api+'/api/bookings',{method:'POST',headers:{'Content-Type':'application/json',...authHeaders()},body:JSON.stringify({billboardId:selected.id,durationMinutes:bookingMinutes,companyName:bookingCompanyName||user.displayName||user.username,description:adTitle.trim()||undefined,advertisementId:advertisementId||undefined})});const data=await readApi(r);if(!r.ok)throw new Error(data.error||'Booking failed');setBalance(data.balance);setBidders(v=>({...v,[selected.id]:{name:data.booking.companyName,amount:Number(data.booking.amount),siteUrl:adUrl||data.booking.targetUrl||data.booking.user?.websiteUrl||user.websiteUrl||undefined,imageUrl:data.booking.advertisement?.imageUrl||undefined,description:data.booking.description||data.booking.advertisement?.description||adTitle||undefined}}));setActiveBookings(v=>({...v,[selected.id]:data.booking}));selected.occupied=true;setSelected({...selected});socket.current?.emit('billboard:book',{id:selected.id,amount:Number(data.booking.amount),bidder:{name:data.booking.companyName,amount:Number(data.booking.amount)}});await loadHistory();alert('Advertising space booked successfully for '+formatDuration(bookingMinutes));}catch(e:any){setBookingError(e.message||'Booking failed')}finally{setBookingBusy(false)}};

 const uploadImageOnly=async()=>{if(!adFile)return undefined;const fd=new FormData();fd.append('file',adFile);const r=await fetch(api+'/api/advertisements/upload',{method:'POST',headers:authHeaders(),body:fd});const d=await readApi(r);if(!r.ok)throw new Error(d.error||'Upload failed');return d.imageUrl;};
 const saveCreative=async()=>{if(!selected||!user)return;setEditBusy(true);setBookingError('');try{const imageUrl=removePhoto?null:await uploadImageOnly();const body:any={companyName:bookingCompanyName.trim()||undefined,description:adTitle.trim(),targetUrl:adUrl.trim()};if(removePhoto)body.imageUrl=null;else if(imageUrl)body.imageUrl=imageUrl;const r=await fetch(api+'/api/bookings/'+encodeURIComponent(selected.id)+'/creative',{method:'PATCH',headers:{'Content-Type':'application/json',...authHeaders()},body:JSON.stringify(body)});const data=await readApi(r);if(!r.ok)throw new Error(data.error||'Could not update creative');setActiveBookings(v=>({...v,[selected.id]:data}));setBidders(v=>({...v,[selected.id]:{name:data.companyName,amount:Number(data.amount||0),siteUrl:data.targetUrl||data.siteUrl||undefined,imageUrl:toAssetUrl(data.imageUrl),description:data.description||undefined}}));setAdFile(null);setRemovePhoto(false);setEditMode(false);await loadAllActiveBillboards();}catch(e:any){setBookingError(e.message||'Could not update creative')}finally{setEditBusy(false)}};
 ...
 return <div className="app"><World setNearby={setNearby} players={players} setSelected={setSelected} onMove={(state)=>socket.current?.emit('player:update',state)} timeMode={timeMode} visitorStats={visitorStats} onLocalPosition={setLocalPosition} bidders={bidders}/>
  ...
  {selected&&<div className="panel"><button className="close" onClick={()=>setSelected(null)}>×</button>{(()=>{const active=activeBookings[selected.id];const bidder=bidders[selected.id];const companyName=bidder?.name||active?.companyName||'Company name';const siteUrl=bidder?.siteUrl||active?.targetUrl||active?.siteUrl||active?.user?.websiteUrl;const description=bidder?.description||active?.description||active?.advertisement?.description||active?.user?.companyDescription||'Company description will appear here when this space is booked.';return <><h2>{siteUrl?<a href={siteUrl} target="_blank" rel="noreferrer" className="company-link">{companyName} <span aria-hidden="true">↗</span></a>:companyName}</h2><p>{description}</p></>})()}<div className="tag">{selected.traffic} Traffic</div><div className="stat"><span>Footfall</span><b>{visitorStats[selected.id]||0}</b></div>{activeBookings[selected.id]&&<><div className="stat"><span>Ends</span><b>{shortDate(activeBookings[selected.id].endDate)}</b></div><div className="stat"><span>Time remaining</span><b>{remaining(activeBookings[selected.id].endDate)}</b></div></>}{activeBookings[selected.id]?.userId===user?.id&&<div style={{margin:'12px 0'}}>{!editMode?<button className="bid" onClick={()=>setEditMode(true)}>Edit My Board</button>:<div style={{padding:'10px',border:'1px solid rgba(143,240,179,.35)',borderRadius:10}}><b>Edit your active advertisement</b><input value={bookingCompanyName} onChange={e=>setBookingCompanyName(e.target.value)} placeholder="Name" style={{width:'100%',boxSizing:'border-box',marginTop:8,padding:9,borderRadius:7,border:'1px solid #51627b',background:'#0a101a',color:'#fff'}}/><input value={adTitle} onChange={e=>setAdTitle(e.target.value)} placeholder="Description" style={{width:'100%',boxSizing:'border-box',marginTop:8,padding:9,borderRadius:7,border:'1px solid #51627b',background:'#0a101a',color:'#fff'}}/><input value={adUrl} onChange={e=>setAdUrl(e.target.value)} placeholder="Link (optional)" type="url" style={{width:'100%',boxSizing:'border-box',marginTop:8,padding:9,borderRadius:7,border:'1px solid #51627b',background:'#0a101a',color:'#fff'}}/><input type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>{setAdFile(e.target.files?.[0]||null);setRemovePhoto(false)}} style={{width:'100%',marginTop:8}}/><label style={{display:'flex',gap:8,alignItems:'center',marginTop:8}}><input type="checkbox" checked={removePhoto} onChange={e=>{setRemovePhoto(e.target.checked);if(e.target.checked)setAdFile(null)}}/> Remove photo and show text only</label><div style={{display:'flex',gap:8,marginTop:10}}><button className="bid" onClick={saveCreative} disabled={editBusy}>{editBusy?'Saving...':'Save Changes'}</button><button className="duration-step" onClick={()=>setEditMode(false)}>Cancel</button></div></div>}</div>}<div style={{margin:'12px 0'}}><b>Booking duration</b><div className="duration-picker"><button className="duration-step" onClick={()=>setBookingMinutes(m=>Math.max(30,m-30))}>−</button><b className="duration-value">{formatDuration(bookingMinutes)}</b><button className="duration-step" onClick={()=>setBookingMinutes(m=>Math.min(2880,m+30))}>+</button><button className="day-package" onClick={()=>setBookingMinutes(1440)}><strong>1 day</strong><small>$8.40</small></button></div><small>Maximum 48 hours • 30-minute steps</small></div><div style={{margin:'12px 0',padding:'10px',border:'1px solid rgba(255,255,255,.12)',borderRadius:10}}><b>Advertisement creative</b><input value={bookingCompanyName} onChange={e=>setBookingCompanyName(e.target.value)} placeholder="Name" style={{width:'100%',boxSizing:'border-box',marginTop:8,padding:9,borderRadius:7,border:'1px solid #51627b',background:'#0a101a',color:'#fff'}}/><input value={adTitle} onChange={e=>setAdTitle(e.target.value)} placeholder="Description (optional)" style={{width:'100%',boxSizing:'border-box',marginTop:8,padding:9,borderRadius:7,border:'1px solid #51627b',background:'#0a101a',color:'#fff'}}/><input value={adUrl} onChange={e=>setAdUrl(e.target.value)} placeholder="Link (optional)" type="url" style={{width:'100%',boxSizing:'border-box',marginTop:6,padding:9,borderRadius:7,border:'1px solid #51627b',background:'#0a101a',color:'#fff'}}/><input type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>setAdFile(e.target.files?.[0]||null)} style={{width:'100%',marginTop:8}}/><small>{adFile?adFile.name:'PNG, JPG or WEBP • max 5 MB'}</small></div><div className="stat"><span>Fixed price</span><b>{formatUsd(bookingPrice(selected,bookingMinutes))}</b></div><small>{isMain(selected)?'Main billboard: $0.21/30 min • 1-day package $8.40 • after 24 hours $0.19/30 min':'Side/corner/wall billboard: $1.05 per 30 minutes'}</small>{bookingError&&<p style={{color:'#ff8f8f'}}>{bookingError}</p>}<button className="bid" onClick={book} disabled={bookingBusy||uploadBusy}>{user?((bookingBusy||uploadBusy)?(uploadBusy?'Uploading...':'Booking...'):'Book This Space'):'Login to Book'}</button></div>}
