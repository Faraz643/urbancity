...
type AuthUser={id:string;email:string;username:string;displayName:string;role:string;wallet?:{balance:number}|null};
...
function App(){
 ...
 const formatUsd=(value:number)=>' const [bookingBusy,setBookingBusy]=useState(false); const [bookingError,setBookingError]=useState(''); const [historyOpen,setHistoryOpen]=useState(false); const [bookingHistory,setBookingHistory]=useState<any[]>([]); const [leaderboard,setLeaderboard]=useState<any[]>([]); const [activeBookings,setActiveBookings]=useState<Record<string,any>>({}); const [clock,setClock]=useState(Date.now());
 ...
 const submitAuth=async()=>{setAuthError('');setAuthBusy(true);try{const body=authMode==='login'?{email:authEmail,password:authPassword}:{email:authEmail,password:authPassword,username:authUsername,displayName:authUsername};const r=await fetch(api+'/api/auth/'+(authMode==='login'?'login':'register'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const data=await readApi(r);if(!r.ok)throw new Error(data.error||'Authentication failed');localStorage.setItem('urbancity_token',data.token);await loadMe();setAuthOpen(false);setAuthPassword('');}catch(e:any){setAuthError(e.message||'Authentication failed');}finally{setAuthBusy(false);}};
...
+value.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2});
 const [bookingMinutes,setBookingMinutes]=useState(30); const [bookingBusy,setBookingBusy]=useState(false); const [bookingError,setBookingError]=useState(''); const [historyOpen,setHistoryOpen]=useState(false); const [bookingHistory,setBookingHistory]=useState<any[]>([]); const [leaderboard,setLeaderboard]=useState<any[]>([]); const [activeBookings,setActiveBookings]=useState<Record<string,any>>({}); const [clock,setClock]=useState(Date.now());
 const authInputRef=useRef<HTMLInputElement|null>(null);
 const totalVisitors=players.length+1;
 const visitorStats=useMemo(()=>{const stats:Record<string,number>={};for(const b of MAP_BILLBOARDS)stats[b.id]=0;const all=[localPosition,...players.map(p=>p.position)];for(const pos of all){for(const b of MAP_BILLBOARDS){const radius=b.kind==='wall-ad'?6:7;if(Math.hypot(pos[0]-b.position[0],pos[2]-b.position[2])<=radius)stats[b.id]++}}return stats},[players,localPosition]);
 ...
 const isMain=(b:Billboard)=>b.type==='Premium Road';
 const bookingPrice=(b:Billboard,m:number)=>{if(!isMain(b))return m/30*100;if(m===1440)return 800;if(m>1440)return 800+((m-1440)/30)*18;return (m/30)*20;};
 const remaining=(end?:string)=>{if(!end)return '';let s=Math.max(0,Math.ceil((new Date(end).getTime()-clock)/1000));const d=Math.floor(s/86400);s%=86400;const h=Math.floor(s/3600);s%=3600;const m=Math.floor(s/60);s%=60;return (d?d+'d ':'')+String(h).padStart(2,'0')+'h '+String(m).padStart(2,'0')+'m '+String(s).padStart(2,'0')+'s'};
 const formatDuration=(m:number)=>{if(m<60)return '30 min';const hours=m/60;return Number.isInteger(hours)?hours+' hour'+(hours===1?'':'s'):hours+' hours'};
 const loadHistory=async()=>{try{const r=await fetch(api+'/api/bookings/history');const d=await readApi(r);if(r.ok)setBookingHistory(d)}catch{}};
 const loadLeaderboard=async()=>{try{const r=await fetch(api+'/api/bookings/leaderboard');const d=await readApi(r);if(r.ok)setLeaderboard(Array.isArray(d)?d:[])}catch{}};
 ...
 return <div className="app"><World setNearby={setNearby} players={players} setSelected={setSelected} onMove={(state)=>socket.current?.emit('player:update',state)} timeMode={timeMode} visitorStats={visitorStats} onLocalPosition={setLocalPosition} bidders={bidders}/>
  ...
  <MiniMap players={players}/><div className="billcount">🪧 Billboards <b>{MAP_BILLBOARDS.length}</b> total</div>
  {nearby&&!selected&&<button className="interact" onClick={()=>setSelected(nearby)}><kbd>E</kbd><span>Interact</span></button>}
  {selected&&<div className="panel"><button className="close" onClick={()=>setSelected(null)}>×</button><h2>{selected.kind==='wall-ad'?'Wall Ad':'Billboard'} #{selected.id}</h2><p>Fixed-price advertising space • {selected.type}</p><div className="tag">{selected.traffic} Traffic</div><div className="stat"><span>Currently displayed</span><b>{bidders[selected.id]?.name||'Available'}</b></div><div className="stat"><span>Visitors Near This Ad</span><b>{visitorStats[selected.id]||0}</b></div>{activeBookings[selected.id]&&<><div className="stat"><span>Ends</span><b>{shortDate(activeBookings[selected.id].endDate)}</b></div><div className="stat"><span>Time remaining</span><b>{remaining(activeBookings[selected.id].endDate)}</b></div></>}<div style={{margin:'12px 0'}}><b>Booking duration</b><div className="duration-picker"><button className="duration-step" onClick={()=>setBookingMinutes(m=>Math.max(30,m-30))}>−</button><b className="duration-value">{formatDuration(bookingMinutes)}</b><button className="duration-step" onClick={()=>setBookingMinutes(m=>Math.min(2880,m+30))}>+</button><button className="day-package" onClick={()=>setBookingMinutes(1440)}><strong>1 day</strong><small>₹800</small></button></div><small>Maximum 48 hours • 30-minute steps</small></div><div className="stat"><span>Fixed price</span><b>₹{bookingPrice(selected,bookingMinutes).toLocaleString()}</b></div><small>{isMain(selected)?'Main billboard: ₹20/30 min • 1-day package ₹800 (save ₹160) • after 24 hours ₹18/30 min':'Side/corner/wall billboard: ₹100 per 30 minutes'}</small>{bookingError&&<p style={{color:'#ff8f8f'}}>{bookingError}</p>}<button className="bid" onClick={book} disabled={bookingBusy}>{user?(bookingBusy?'Booking...':'Book This Space'):'Login to Book'}</button></div>}
  {historyOpen&&<div className="leaderboard-overlay"><section className="leaderboard-window"><button className="leaderboard-close" onClick={()=>setHistoryOpen(false)}>×</button><div className="leaderboard-head"><span>URBANCITY</span><h1>Leaderboard</h1><p>Top advertisers by total spend</p></div><div className="leaderboard-list">{leaderboard.length===0?<div className="empty-state">No advertisers yet.</div>:leaderboard.map((x:any)=><article className="leaderboard-row" key={x.username+'-'+x.name}><div className="rank">#{x.rank}</div><div className="company-logo"><img src={x.logo||'/company-placeholder.svg'} alt={x.name+' logo'}/></div><div className="company-main"><b>{x.name}</b><small>@{x.username}</small></div><div className="leaderboard-metric"><small>Total paid</small><b>₹{Number(x.totalPayment).toLocaleString()}</b></div><div className="leaderboard-metric"><small>Total time</small><b>{durationLabel(x.totalMinutes)}</b></div><a className="site-link" href={x.siteUrl}>Site ↗</a></article>)}</div></section></div>}
