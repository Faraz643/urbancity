...
function App(){
 ...
 return <div className="app"><World setNearby={setNearby} players={players} setSelected={setSelected} onMove={(state)=>socket.current?.emit('player:update',state)} timeMode={timeMode} visitorStats={visitorStats} onLocalPosition={setLocalPosition}/>
  ...
  {authOpen&&<div style={{position:'fixed',inset:0,zIndex:100,background:'rgba(3,7,14,.78)',display:'grid',placeItems:'center',backdropFilter:'blur(8px)'}}><div style={{width:360,maxWidth:'90vw',background:'#111a28',border:'1px solid #4c5d74',borderRadius:16,padding:24,color:'#fff',boxShadow:'0 20px 70px #000'}}><button onClick={()=>setAuthOpen(false)} style={{float:'right',background:'transparent',border:0,color:'#fff',fontSize:22,cursor:'pointer'}}>×</button><h2 style={{marginTop:0}}>UrbanCity Account</h2><p style={{color:'#aeb9c8'}}>{authMode==='login'?'Login to bid on real advertising inventory.':'Create an account and receive ₹100,000 virtual auction balance.'}</p>{authMode==='register'&&<input value={authUsername} onChange={e=>setAuthUsername(e.target.value)} placeholder="Username" style={{width:'100%',boxSizing:'border-box',padding:12,margin:'6px 0',borderRadius:8,border:'1px solid #51627b',background:'#0a101a',color:'#fff'}}/>}<input value={authEmail} onChange={e=>setAuthEmail(e.target.value)} placeholder="Email" type="email" style={{width:'100%',boxSizing:'border-box',padding:12,margin:'6px 0',borderRadius:8,border:'1px solid #51627b',background:'#0a101a',color:'#fff'}}/><input value={authPassword} onChange={e=>setAuthPassword(e.target.value)} placeholder="Password" type="password" style={{width:'100%',boxSizing:'border-box',padding:12,margin:'6px 0',borderRadius:8,border:'1px solid #51627b',background:'#0a101a',color:'#fff'}}/>{authError&&<p style={{color:'#ff8f8f'}}>{authError}</p>}<button onClick={submitAuth} disabled={authBusy} style={{width:'100%',padding:12,marginTop:10,border:0,borderRadius:8,background:'#e5b75b',fontWeight:800,cursor:'pointer'}}>{authBusy?'Please wait...':authMode==='login'?'Login':'Create Account'}</button><button onClick={()=>{setAuthMode(authMode==='login'?'register':'login');setAuthError('')}} style={{width:'100%',padding:10,marginTop:8,border:'1px solid #51627b',borderRadius:8,background:'transparent',color:'#dbe5f3',cursor:'pointer'}}>{authMode==='login'?'Need an account? Register':'Already have an account? Login'}</button></div></div>}
 </div>
}
export default App;
