...
function App(){
 ...
 const loadActiveBillboard=async(id:string)=>{try{const r=await fetch(api+'/api/bookings/billboard/'+encodeURIComponent(id));const d=await readApi(r);if(r.ok&&d.active){setActiveBookings(v=>({...v,[id]:d.active}));setBidders(v=>({...v,[id]:{name:d.active.companyName||d.active.user?.displayName||d.active.user?.username||'Advertiser',amount:Number(d.active.amount||0),siteUrl:d.active.siteUrl||d.active.user?.websiteUrl||undefined}}));}}catch{}};
