...
function App(){
 ...
 const bookingPrice=(b:Billboard,m:number)=>{if(isMain(b)&&m===720)return 3000;if(isMain(b)&&m>720)return m/30*125;return m/30*(isMain(b)?150:100)};
 ...
 const formatDuration=(m:number)=>m>=1440?(m/1440)+' day'+(m===1440?'':'s'):m>=60?(m/60)+' hour'+(m===60?'':'s'):'30 minutes';
 ...
 return <div className="app"><World setNearby={setNearby} players={players} setSelected={setSelected} onMove={(state)=>socket.current?.emit('player:update',state)} timeMode={timeMode} visitorStats={visitorStats} onLocalPosition={setLocalPosition} bidders={bidders}/>
  ...
  <button onClick={()=>{setHistoryOpen(v=>!v);loadHistory()}} style={{position:'fixed',top:68,right:18,zIndex:20,border:'1px solid #6d7b91',background:'#111a28',color:'#fff',borderRadius:8,padding:'10px 14px',fontWeight:700,cursor:'pointer'}}>Company History</button><button onClick={()=>user?logout():setAuthOpen(true)} style={{position:'fixed',top:18,right:18,zIndex:20,border:'1px solid #6d7b91',background:'#111a28',color:'#fff',borderRadius:8,padding:'10px 14px',fontWeight:700,cursor:'pointer'}}>{user?'Logout '+user.username:'Login / Register'}</button>
  ...
  {selected&&<div className="panel"><button className="close" onClick={()=>setSelected(null)}>×</button><h2>{selected.kind==='wall-ad'?'Wall Ad':'Billboard'} #{selected.id}</h2><p>Fixed-price advertising space • {selected.type}</p><div className="tag">{selected.traffic} Traffic</div><div className="stat"><span>Currently displayed</span><b>{bidders[selected.id]?.name||'Available'}</b></div><div className="stat"><span>Visitors Near This Ad</span><b>{visitorStats[selected.id]||0}</b></div>{activeBookings[selected.id]&&<><div className="stat"><span>Booking ends</span><b>{new Date(activeBookings[selected.id].endDate).toLocaleString()}</b></div><div className="stat"><span>Time remaining</span><b>{remaining(activeBookings[selected.id].endDate)}</b></div></>}<div style={{margin:'12px 0'}}><b>Booking duration</b><div style={{display:'flex',alignItems:'center',gap:10,marginTop:8}}><button onClick={()=>setBookingMinutes(m=>Math.max(30,m-30))}>−</button><b style={{minWidth:100,textAlign:'center'}}>{formatDuration(bookingMinutes)}</b><button onClick={()=>setBookingMinutes(m=>Math.min(2880,m+30))}>+</button><button onClick={()=>setBookingMinutes(1440)} style={{marginLeft:'auto'}}>1 day</button></div><small>Maximum 2 days • 30-minute steps</small></div><div className="stat"><span>Fixed price</span><b>₹{bookingPrice(selected,bookingMinutes).toLocaleString()}</b></div><small>{isMain(selected)?'Main billboard: ₹150/30 min • 12 hours = ₹3,000 • after 12 hours ₹125/30 min':'Side/corner/wall billboard: ₹100 per 30 minutes'}</small>{bookingError&&<p style={{color:'#ff8f8f'}}>{bookingError}</p>}<button className="bid" onClick={book} disabled={bookingBusy}>{user?(bookingBusy?'Booking...':'Book This Space'):'Login to Book'}</button></div>}
  {historyOpen&&<aside style={{position:'fixed',top:18,right:18,width:340,maxHeight:'82vh',overflow:'auto',zIndex:80,background:'#101925',border:'1px solid #4c5d74',borderRadius:14,padding:18,color:'#fff'}}><button onClick={()=>setHistoryOpen(false)} style={{float:'right'}}>×</button><h2>Advertiser History</h2><p style={{color:'#aeb9c8'}}>Companies and startups that have booked UrbanCity advertising.</p>{bookingHistory.length===0?<p>No completed bookings yet.</p>:bookingHistory.map(x=><div key={x.id} style={{borderTop:'1px solid #334155',padding:'10px 0'}}><b>{x.companyName}</b><div>{x.billboard?.name||'Billboard '+x.billboardId}</div><small>Spent ₹{Number(x.amount).toLocaleString()} • {formatDuration(x.durationMinutes)}</small></div>)}</aside>}
