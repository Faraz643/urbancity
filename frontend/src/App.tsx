 ...
 const submitAuth=async()=>{
  ...
  if(authMode==='register'){
    ...
    if(website){
      try{
        ...
        return setAuthError('Please enter a valid website URL, including https:// (for example: https://yourcompany.com).');
      }
    }
  }
  setAuthBusy(true);
  try{
    ...
    if(!r.ok){
      ...
      if(authMode==='login'){
        if(r.status===404||serverMessage.includes('not found')||serverMessage.includes('no user')) throw new Error('No account was found with this email address. Please check your email or create an account.');
        if(r.status===401||serverMessage.includes('invalid')||serverMessage.includes('password')||serverMessage.includes('credential')) throw new Error('Incorrect email or password. Please try again.');
      }else{
        if(r.status===409||serverMessage.includes('already exists')||serverMessage.includes('already registered')||serverMessage.includes('duplicate')) throw new Error('An account with these details already exists. Please log in instead.');
      }
    ...
    if(e instanceof TypeError) setAuthError('Unable to connect to the server. Please check your internet connection and try again.');
    else setAuthError(e.message||'We could not complete your request. Please try again.');
