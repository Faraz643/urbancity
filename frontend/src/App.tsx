...
function App(){
 ...
 const [adFile,setAdFile]=useState<File|null>(null),[adTitle,setAdTitle]=useState(''),[adUrl,setAdUrl]=useState(''),[bookingCompanyName,setBookingCompanyName]=useState(''),[uploadBusy,setUploadBusy]=useState(false);
 ...
 const visitorStats=useMemo(()=>{
   ...
   for(const pos of all){
     for(const b of MAP_BILLBOARDS){
       const radius=b.kind==='wall-ad'?6:7;
 ...
 const loadAllActiveBillboards=async()=>{for(const b of MAP_BILLBOARDS)await loadActiveBillboard(b.id);};
 const loadActiveBillboard=async(id:string)=>{setBookingError('');try{const r=await fetch(api+'/api/bookings/billboard/'+encodeURIComponent(id));const d=await readApi(r);if(r.ok&&d.active){setActiveBookings(v=>({...v,[id]:d.active}));setBidders(v=>({...v,[id]:{name:d.active.companyName||d.active.user?.displayName||d.active.user?.username||'Advertiser',amount:Number(d.active.amount||0),siteUrl:d.active.targetUrl||d.active.siteUrl||d.active.user?.websiteUrl||undefined,imageUrl:d.active.imageUrl||undefined}}));}else{setActiveBookings(v=>{const n={...v};delete n[id];return n});setBidders(v=>{const n={...v};delete n[id];return n});}}catch{setActiveBookings(v=>{const n={...v};delete n[id];return n});}};
 useEffect(()=>{if(selected?.id)loadActiveBillboard(selected.id);},[selected?.id]);
 useEffect(()=>{loadAllActiveBillboards();const t=window.setInterval(loadAllActiveBillboards,30000);return()=>window.clearInterval(t)},[]);
