...
function App(){
 ...
 useEffect(()=>{const params=new URLSearchParams(window.location.search);const bookingId=params.get('booking');const orderId=params.get('order_id');if(params.get('payment')!=='return'||!bookingId||!orderId||!localStorage.getItem('urbancity_token'))return;let cancelled=false;(async()=>{try{const r=await fetch(api+'/api/payments/'+encodeURIComponent(bookingId)+'/verify?order_id='+encodeURIComponent(orderId),{headers:authHeaders()});const data=await readApi(r);if(cancelled)return;if(!r.ok)throw new Error(data.error||'Could not verify payment');if(data.paid){setBookingError('Payment successful. Your advertising space is now active.');await loadAllActiveBillboards();}else{setBookingError(data.providerStatus==='ACTIVE'?'Payment is still pending confirmation. Please refresh in a moment.':'Payment was not completed.');}window.history.replaceState({},'',window.location.pathname);}catch(e:any){if(!cancelled)setBookingError(e.message||'Could not verify payment');}})();return()=>{cancelled=true}},[api]);
