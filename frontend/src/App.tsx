...
function App(){
 ...
 const [paymentNotice,setPaymentNotice]=useState<{message:string;ok:boolean}|null>(null);
 ...
 useEffect(()=>{const params=new URLSearchParams(window.location.search);const bookingId=params.get('booking');const orderId=params.get('order_id');if(params.get('payment')!=='return'||!bookingId||!orderId||!localStorage.getItem('urbancity_token'))return;let cancelled=false;(async()=>{try{const r=await fetch(api+'/api/payments/'+encodeURIComponent(bookingId)+'/verify?order_id='+encodeURIComponent(orderId),{headers:authHeaders()});const data=await readApi(r);if(cancelled)return;if(!r.ok)throw new Error(data.error||'Could not verify payment');if(data.paid){setPaymentNotice({message:'Payment successful. Your advertising space is now active.',ok:true});await loadAllActiveBillboards();}else{setPaymentNotice({message:data.providerStatus==='ACTIVE'?'Payment is still pending confirmation. Please refresh in a moment.':'Payment was not completed.',ok:false});}window.history.replaceState({},'',window.location.pathname);}catch(e:any){if(!cancelled)setPaymentNotice({message:e.message||'Could not verify payment',ok:false});}})();return()=>{cancelled=true}},[api]);
 ...
 const bookingPrice=(b:Billboard,m:number)=>{if(!isMain(b))return (m/30)*105;if(m===1440)return 840;if(m>1440)return 840+((m-1440)/30)*19;return (m/30)*21;};
