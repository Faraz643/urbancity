...
function validateProductionEnvironment(){if(process.env.NODE_ENV!=='production')return;getJwtSecret();const required=['DATABASE_URL','FRONTEND_URL','CASHFREE_CLIENT_ID','CASHFREE_CLIENT_SECRET','CASHFREE_CUSTOMER_PHONE','CASHFREE_NOTIFY_URL','DODO_PAYMENTS_API_KEY','DODO_PAYMENTS_PRODUCT_ID','DODO_PAYMENTS_WEBHOOK_KEY'];const missing=required.filter(k=>!process.env[k]);if(missing.length)throw new Error('Missing required production environment variables: '+missing.join(', '));}
