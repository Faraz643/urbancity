...
function syncPlayerHeight(player:Player) {
  player.height=Number(playerGrowthHeight(player).toFixed(3));
...
io.on('connection',(socket)=>{
  const player:Player={id:socket.id,name:socket.data.userId?'Player-'+String(socket.data.userId).slice(0,4):'Visitor-'+randomUUID().slice(0,4),position:[0,1,8],rotation:0,moving:false,joinedAt:Date.now(),height:PLAYER_BASE_HEIGHT};
  players.set(socket.id,player);
  socket.emit('players:list',[...players.values()].map(p=>({...p,height:syncPlayerHeight(p)})));
  socket.emit('player:self',({...player,height:syncPlayerHeight(player)}));
  socket.broadcast.emit('player:joined',{...player,height:syncPlayerHeight(player)});
  ...
  socket.on('player:update',(data:Partial<Player>)=>{
    ...
    syncPlayerHeight(current);
    const snapshot={...current};
    socket.emit('player:self',snapshot);
    socket.broadcast.emit('player:update',snapshot);
  });
