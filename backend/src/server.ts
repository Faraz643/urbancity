...
type Player = { id:string; name:string; position:[number,number,number]; rotation:number; moving:boolean; height:number };
const PLAYER_BASE_HEIGHT=2.8;
const PLAYER_MAX_HEIGHT=44;
const PLAYER_GROWTH_PER_SECOND=0.01;
const players = new Map<string, Player>();
const playerStartedAt = new Map<string, number>();
...
function calculatePlayerHeight(playerId:string, now=Date.now()) {
  const startedAt=playerStartedAt.get(playerId) ?? now;
  return Math.min(PLAYER_MAX_HEIGHT, PLAYER_BASE_HEIGHT + Math.max(0,now-startedAt)/1000*PLAYER_GROWTH_PER_SECOND);
}
...
io.on('connection',(socket)=>{
  const now=Date.now();
  playerStartedAt.set(socket.id,now);
  const player:Player={id:socket.id,name:socket.data.userId?'Player-'+String(socket.data.userId).slice(0,4):'Visitor-'+randomUUID().slice(0,4),position:[0,1,8],rotation:0,moving:false,height:PLAYER_BASE_HEIGHT};
  ...
  socket.on('player:update',(data:Partial<Player>)=>{
    const current=players.get(socket.id);if(!current)return;
    if(Array.isArray(data.position)&&data.position.length===3&&data.position.every(v=>typeof v==='number'&&Number.isFinite(v))){const next=data.position as [number,number,number];const dx=next[0]-current.position[0],dy=next[1]-current.position[1],dz=next[2]-current.position[2];if(Math.hypot(dx,dy,dz)<=8&&Math.abs(next[0])<=80&&next[1]>=-2&&next[1]<=30&&Math.abs(next[2])<=80)current.position=next}
    ...
    // Height is server-authoritative. A client cannot speed up growth by sending a larger value.
    current.height=calculatePlayerHeight(socket.id);
  ...
  socket.on('disconnect',()=>{players.delete(socket.id);playerStartedAt.delete(socket.id);playerFootfallInside.delete(socket.id);io.emit('player:left',socket.id);io.emit('online:count',players.size)});
});
